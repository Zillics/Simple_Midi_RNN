...
def midiToMatrix(filename,sample_interval_ms):
	midi_data = pretty_midi.PrettyMIDI(filename)
	interval_s = sample_interval_ms*0.00002

	piano_roll = midi_data.get_piano_roll(fs=sample_interval_ms, times=None)
...
def matrixToMidi(matrix,filename,interval_ms):
	# Create a PrettyMIDI object
	midi_obj = pretty_midi.PrettyMIDI()
	# Create an Instrument instance for a Piano instrument
	piano_program = pretty_midi.instrument_name_to_program('Acoustic Grand Piano')
	piano = pretty_midi.Instrument(program=piano_program)
	interval_s = interval_ms*0.00002
	...
	for i in range(0,n_notes):
		...
		e = s + matrix[i,1] # End = start + duration of note
...
if __name__ == '__main__':
	matrix = midiToMatrix('aria_melody.mid',200)
	print(matrix.shape)
	matrixToMidi(matrix,'aria_pretty.mid',200)